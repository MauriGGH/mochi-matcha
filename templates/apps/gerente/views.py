...
from apps.menu.models import Categoria, Producto, GrupoModificador, OpcionModificador, Promocion, TipoPromocion
...
def mesas_crud(request):
    if request.method == "POST":
        ...
        qr = f"mesa-{numero}-{_uuid.uuid4().hex[:8]}"  # QR usa numero para legibilidad
...
def promociones(request):
    if request.method == "POST":
        # FIX: implementar creación de promoción
        titulo = request.POST.get("titulo", "").strip()
        tipo_id = request.POST.get("tipo_promocion_id")
        valor = request.POST.get("valor", 0)
        fecha_inicio = request.POST.get("fecha_inicio")
        fecha_fin = request.POST.get("fecha_fin")
        if titulo and tipo_id and valor and fecha_inicio and fecha_fin:
            tipo = get_object_or_404(TipoPromocion, pk=tipo_id)
            Promocion.objects.create(
                titulo=titulo, tipo_promocion=tipo, valor=valor,
                fecha_inicio=fecha_inicio, fecha_fin=fecha_fin, activa=True,
            )
...
def reportes(request):
    ...
    # FIX: excluir pedidos cancelados del cómputo de ventas
    pedidos = Pedido.objects.filter(
        fecha_hora_ingreso__date__gte=desde
    ).exclude(estado="cancelado").prefetch_related("detalles__producto").select_related("sesion__mesa").order_by("-fecha_hora_ingreso")
...
def configuracion(request):
    if request.method == "POST":
        ...
        return JsonResponse({"ok": True})
    listos_count = Pedido.objects.filter(estado="listo").count()
    return render(request, "gerente/configuracion.html", {"listos_count": listos_count})
