...
def producto_eliminar(request, id):
    """Toggle disponibilidad del producto (soft-delete / reactivación). Nunca hard-delete."""
    producto = get_object_or_404(Producto, pk=id)
    producto.disponible = not producto.disponible
    producto.save(update_fields=["disponible"])
    accion = "Producto habilitado" if producto.disponible else "Producto deshabilitado"
    Auditoria.objects.create(
        accion=accion,
        detalle=f"Producto #{producto.pk} '{producto.nombre}'",
        empleado=request.user,
    )
